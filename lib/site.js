...
export function buildPageMetadata({ title, description, path = '/', image, type = 'website' }) {
  ...
  const usesBrandInTitle =
    typeof fullTitle === 'string' &&
    [siteConfig.name, siteConfig.title].some((brandTitle) => fullTitle.toLowerCase().includes(brandTitle.toLowerCase()));
  const metadataTitle = usesBrandInTitle ? { absolute: fullTitle } : fullTitle;
